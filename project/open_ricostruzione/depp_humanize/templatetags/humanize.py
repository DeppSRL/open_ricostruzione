...
def _get_the_word(value, float_formatted, string_formatted):
    """
    returns only the word classifying the number: millions, billions, ecc
    """
    if settings.USE_L10N:
        value = ''
        template = string_formatted
...
def onlynumber(value):
    """
    returns only the rounded numeric value
    """
    try:
        value = int(value)
    except (TypeError, ValueError):
    ...
    for exponent, converters in intword_converters:
        large_number = 10 ** exponent
        if value < large_number * 1000:
            new_value = round(value / float(large_number))
            return iround(new_value)
    return value
...
def onlyword(value):
    """
    returns only the words for the number category (millions, billions, ecc)
    """
    try:
        value = int(value)
    except (TypeError, ValueError):
    ...
    for exponent, converters in intword_converters:
        ...
        if value < large_number * 1000:
            new_value = value / float(large_number)
            return _get_the_word(new_value, *converters(new_value))
    return value
